...
public abstract class CountOfRangeSum {
	...
	public static void main(String[] args) {
		CountOfRangeSum instance = new SolutionIV();
		int[] nums;
		int lower, upper;
...
//		upper = 0;
	...
	static class SolutionIV extends CountOfRangeSum {
		public int countRangeSum(int[] nums, int lower, int upper) {
	        ...
	        for (int num : nums) {
	            sum += num;
	            count += sub(sums, sum-upper, sum-lower);
	            sums.add(sum);
	    ...
	    private int sub(List<Integer> list, int lower, int upper) {
	        int idx1 = left(list, lower, 0, list.size()-1);
	        int idx2 = right(list, upper, 0, list.size()-1);
	        if (idx1 < 0 || idx2 < 0)
	            return 0;
	        return idx2 - idx1 + 1;
	    }
	    
	    // return leftmost idx of num >= target
	    private int left(List<Integer> list, int target, int l, int r) {
	        if (l > r)
	            return -1;
	        int m = (l + r) >> 1;
	        if (list.get(m) >= target && (m-1 < l || list.get(m-1) < target))
	            return m;
	        else if (m-1 >= l && list.get(m-1) >= target)
	            return left(list, target, l, m-1);
	        else
	            return left(list, target, m+1, r);
	    }
	    
	    // return rightmost idx of num <= target
	    private int right(List<Integer> list, int target, int l, int r) {
	        if (l > r)
	            return -1;
	        int m = (l + r) >> 1;
	        if (list.get(m) <= target && (m+1 > r || list.get(m+1) > target))
	            return m;
	        else if (m+1 <= r && list.get(m+1) <= target)
	            return right(list, target, m+1, r);
	        else
	            return right(list, target, l, m-1);
	    }
	}
	
	
	// Solution III: Accepted
	// however, worst case O(n^2*logn)
	static class SolutionIII extends CountOfRangeSum {
		public int countRangeSum(int[] nums, int lower, int upper) {
	        if (nums == null || nums.length == 0)
	            return 0;
	        int result = 0;
	        TreeMap<Long, Integer> sumCount = new TreeMap<>();
	        sumCount.put(0L, 1); // padding on left, where no left sub.
	        long sum = 0;
