...
public class CountCompleteTreeNodes {
	...
	public int countNodes(TreeNode root) {
        ...
        return (int)Math.pow(2, maxH) - 1 - ((int)Math.pow(2, maxH-1) - leaves);
    }
	*/
    ...
    public int countNodes(TreeNode root) {
        ...
        getLeaves(root, 1, h);
        return (int)Math.pow(2, h) - 1 - ((int)Math.pow(2, h-1) - leaves);
    }
    
    // dfs, bump leaves if h==maxH; return false if curr subtree is incomplete.
    // node is never null
    private boolean getLeaves(TreeNode node, int h, int maxH) {
        // termination
        if (node.left == null) {
            if (h == maxH) {
                leaves++;
        ...
        if (getLeaves(node.left, h+1, maxH) && (node.right != null && getLeaves(node.right, h+1, maxH)))
            return true;
        else
            return false;
    }
