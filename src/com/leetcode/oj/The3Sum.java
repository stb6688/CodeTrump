...
public class The3Sum {
	...
	public List<List<Integer>> threeSum(int[] nums) {
        ...
        for (int left = 0; left < nums.length; left++) {
            for (int right = left+2; right < nums.length; right++) {
                ...
                int mid = binary(nums, left+1, right-1, target);
                if (mid > 0) {
	...
	public List<List<Integer>> threeSum(int[] nums) {
        ...
        while (left < right) {
            search(nums, left, right, set);
            search(nums, left+1, right, set);
            search(nums, left, right-1, set);
            left++;
            right--;
        }
        List<List<Integer>> results = new ArrayList<>();
    ...
    private void search(int[] nums, int start, int end, Set<List<Integer>> set) {
        if (start >= end)
            return;
        int target = -nums[start]-nums[end];
        int mid = binary(nums, start+1, end-1, target);
        if (mid >= 0) {
            List<Integer> result = new ArrayList<>(3);
